...
void CURL::Parse(const CStdString& strURL1)
{
  ...
  if(strProtocol2.Equals("http")
    ...
    || strProtocol2.Equals("pfc") // Laureon: Filesystem: Added
  ...
  if (iSlash < 0)
  {
    ...
    if (iColon >= 0 && !m_strProtocol.Equals("pfc")) // Laureon: Added: Filesystem: was by paulopina: Why???
    {
...
const CStdString CURL::GetFileNameWithoutPath() const
{
  ...
  if ((m_strProtocol == "rar" || m_strProtocol == "zip" || m_strProtocol == "pfc") && m_strFileName.IsEmpty()) // Laureon: Added: Filesystem
