...
CGUIJukeboxWindow::CGUIJukeboxWindow(int id, const char *xmlFile) : CGUIWindow(id, xmlFile) 
{
  m_vecItems = new CFileItemList;
  m_vecTracks = new CFileItemList;
  m_vecPlaylist = new CFileItemList;
...
CGUIJukeboxWindow::~CGUIJukeboxWindow()
{
  delete m_vecItems;
  delete m_vecTracks;
  delete m_vecPlaylist;
...
void CGUIJukeboxWindow::LoadAdditionalTags(TiXmlElement *root)
{
  CGUIWindow::LoadAdditionalTags(root);
  // configure our view control
  m_albunsViewControl.Reset();
  m_albunsViewControl.SetParentWindow(GetID());
  TiXmlElement *element = root->FirstChildElement("views");
  // Laureon: Format is...
  //<views>
  //  <albums>58</albums>
  //  <tracks>666</tracks>
  //  <playlist>667</playlist>
  //</views>

  if (element && element->FirstChild()) { // format is <views>50,29,51,95</views>
    CStdString albumsView   = element->FirstChildElement("albums")->Value();
    CStdString tracksView   = element->FirstChildElement("tracks")->Value();
    CStdString playlistView = element->FirstChildElement("playlist")->Value();
    

    CGUIControl *albumsControl = (CGUIControl *)GetControl(atol(albumsView.c_str()));
    if (albumsControl && albumsControl->IsContainer())
      m_albunsViewControl.AddView(albumsControl);

    CGUIControl *tracksControl = (CGUIControl *)GetControl(atol(tracksView.c_str()));
    if (tracksControl && tracksControl->IsContainer())
      m_tracksViewControl.AddView(tracksControl);

    CGUIControl *playlistControl = (CGUIControl *)GetControl(atol(playlistView.c_str()));
    if (playlistControl && playlistControl->IsContainer())
      m_playlistViewControl.AddView(playlistControl);
    ...
    //    m_albunsViewControl.AddView(control);
    //}
  }
  ...
  //  }
  //}
  m_albunsViewControl.SetViewControlID(CONTROL_BTNVIEWASICONS);
  m_tracksViewControl.SetViewControlID(CONTROL_BTNVIEWASICONS);
  m_playlistViewControl.SetViewControlID(CONTROL_BTNVIEWASICONS);
}

void CGUIJukeboxWindow::OnWindowLoaded()
{
...
void CGUIJukeboxWindow::OnWindowUnload()
{
  CGUIWindow::OnWindowUnload();
  m_albunsViewControl.Reset();
  m_tracksViewControl.Reset();
  m_playlistViewControl.Reset();
}

CFileItemPtr CGUIJukeboxWindow::GetCurrentListItem(int offset)
{
  int item = m_albunsViewControl.GetSelectedItem();
...
bool CGUIJukeboxWindow::OnAction(const CAction &action)
{
  ...
  if (action.GetID() == ACTION_CONTEXT_MENU && !m_albunsViewControl.HasControl(GetFocusedControlID()))
  {
    //OnPopupMenu(-1);
    //return true;
  }
...
bool CGUIJukeboxWindow::OnBack(int actionID)
{
...
bool CGUIJukeboxWindow::OnMessage(CGUIMessage& message)
{
  switch ( message.GetMessage() )
  {
  case GUI_MSG_WINDOW_DEINIT:
    {
      m_iSelectedItem = m_albunsViewControl.GetSelectedItem();
  ...
  case GUI_MSG_CLICKED:
    {
      ...
      if (iControl == CONTROL_BTNVIEWASICONS)
      {
        ...
        if (control && control->GetControlType() != CGUIControl::GUICONTROL_BUTTON)
        {
          ...
          OnMessage(msg);
          viewMode = m_albunsViewControl.GetViewModeNumber(msg.GetParam1());
        }
        else
          viewMode = m_albunsViewControl.GetNextViewMode();
      ...
      else if (m_tracksViewControl.HasControl(iControl)) { // Jukebox 666 control
        CFileItemPtr pItem = m_vecTracks->Get(m_tracksViewControl.GetSelectedItem());
        if (!pItem) break;
      ...
      else if (m_albunsViewControl.HasControl(iControl))  // list/thumb control
      {
        CFileItemPtr pItem = m_vecItems->Get(m_albunsViewControl.GetSelectedItem());
        if (!pItem) break;
        int iAction = message.GetParam1();
        if (iAction == ACTION_SELECT_ITEM || iAction == ACTION_MOUSE_LEFT_CLICK)
        {
  ...
  case GUI_MSG_SETFOCUS:
    {
      if (m_albunsViewControl.HasControl(message.GetControlId()) && m_albunsViewControl.GetCurrentControl() != message.GetControlId())
      {
        m_albunsViewControl.SetFocused();
        return true;
      }
    }
    break;
  ...
  case GUI_MSG_NOTIFY_ALL:
    { // Message is received even if this window is inactive
      ...
      else if (message.GetParam1() == GUI_MSG_REMOVED_MEDIA)
      {
        ...
        {
          int iItem = m_albunsViewControl.GetSelectedItem();
          Update(m_vecItems->GetPath());
          m_albunsViewControl.SetSelectedItem(iItem);
        }
  ...
  case GUI_MSG_CHANGE_VIEW_MODE:
    {
      ...
      if (message.GetParam1())  // we have an id
        viewMode = m_albunsViewControl.GetViewModeByID(message.GetParam1());
      else if (message.GetParam2())
        viewMode = m_albunsViewControl.GetNextViewMode((int)message.GetParam2());
...
void CGUIJukeboxWindow::UpdateButtons()
{
  if (m_guiState.get())
  {
    ...
    m_albunsViewControl.SetCurrentView(m_guiState->GetViewAsControl());
...
void CGUIJukeboxWindow::FormatItemLabels(CFileItemList &items, const LABEL_MASKS &labelMasks)
{
...
void CGUIJukeboxWindow::FormatAndSort(CFileItemList &items)
{
...
/*!
  ...
  */
bool CGUIJukeboxWindow::GetDirectory(const CStdString &strDirectory, CFileItemList &items)
{
  // cleanup items
  if (items.Size())
    items.Clear();
...
bool CGUIJukeboxWindow::Update(const CStdString &strDirectory)
{
  ...
  m_albunsViewControl.SetItems(*m_vecItems);
  ...
  for (int i = 0; i < m_vecItems->Size(); ++i)
  {
    ...
    if (!bSelectedFound)
    {
      ...
      {
        m_albunsViewControl.SetSelectedItem(i);
        bSelectedFound = true;
      }
    }
  }
  ...
  if (!bSelectedFound)
    m_albunsViewControl.SetSelectedItem(0);
...
void CGUIJukeboxWindow::OnPrepareFileItems(CFileItemList &items)
{
...
void CGUIJukeboxWindow::OnFinalizeFileItems(CFileItemList &items)
{
...
// \brief With this function you can react on a users click in the list file item.
// It returns true, if the click is handled.
// This function calls OnPlayMedia()
bool CGUIJukeboxWindow::OnClick(CFileItemPtr pItem)
{
  if (pItem->IsParentFolder())
  {
    GoParentFolder();
...
bool CGUIJukeboxWindow::OnSelect(CFileItemPtr pItem)
{
...
bool CGUIJukeboxWindow::HaveDiscOrConnection(const CStdString& strPath, int iDriveType)
{
...
void CGUIJukeboxWindow::ShowShareErrorMessage(CFileItem* pItem)
{
  if (pItem->m_bIsShareOrDrive)
  {
    int idMessageText=0;
...
void CGUIJukeboxWindow::GoParentFolder()
{
  //m_history.DumpPathHistory();
...
void CGUIJukeboxWindow::GetDirectoryHistoryString(const CFileItem* pItem, CStdString& strHistoryString)
{
  if (pItem->m_bIsShareOrDrive)
  {
...
void CGUIJukeboxWindow::SetHistoryForPath(const CStdString& strDirectory)
{
...
bool CGUIJukeboxWindow::OnPlayMedia(CFileItemPtr pItem) {
...
// This function is called by OnClick()
bool CGUIJukeboxWindow::OnPlayAndQueueMedia(const CFileItemPtr &item)
{
  ...
  if (iPlaylist != PLAYLIST_NONE)
  {
    ...
    for ( int i = 0; i < m_vecItems->Size(); i++ )
    {
      ...
      if (!nItem->IsPlayList() && !nItem->IsZIP() && !nItem->IsRAR() && !nItem->IsPFC()) //Laureon: Added: PFS Check
...
void CGUIJukeboxWindow::UpdateFileList()
{
  int nItem = m_albunsViewControl.GetSelectedItem();
  ...
  m_albunsViewControl.SetItems(*m_vecItems);
  m_albunsViewControl.SetSelectedItem(strSelected);
...
void CGUIJukeboxWindow::OnDeleteItem(CFileItemPtr pItem)
{
  if (pItem->IsPlayList())
    pItem->m_bIsFolder = false;
  ...
  if (!CFileUtils::DeleteItem(pItem))
    return;
  m_vecItems->RemoveDiscCache(GetID());
  //Update(m_vecItems->GetPath());
...
void CGUIJukeboxWindow::OnRenameItem(CFileItemPtr pItem)
{
  if (g_settings.GetCurrentProfile().getLockMode() != LOCK_MODE_EVERYONE && g_settings.GetCurrentProfile().filesLocked())
    if (!g_passwordManager.IsMasterLockUnlocked(true))
      return;
...
void CGUIJukeboxWindow::OnInitWindow()
{
  ...
  if (m_iSelectedItem > -1)
    m_albunsViewControl.SetSelectedItem(m_iSelectedItem);

  CGUIWindow::OnInitWindow();
}

CGUIControl *CGUIJukeboxWindow::GetFirstFocusableControl(int id)
{
  if (m_albunsViewControl.HasControl(id))
    id = m_albunsViewControl.GetCurrentControl();
  return CGUIWindow::GetFirstFocusableControl(id);
}

//void CGUIJukeboxWindow::SetupShares()
//{
...
bool CGUIJukeboxWindow::OnPopupMenu(CFileItemPtr pItem) {
...
void CGUIJukeboxWindow::GetContextButtons(CFileItemPtr pItem, CContextButtons &buttons)
{
  if (!pItem)
    return;
  ...
  for (int i = CONTEXT_BUTTON_USER1; i <= CONTEXT_BUTTON_USER10; i++)
  {
    ...
    if (pItem->GetProperty(label).empty())
      break;

    action.Format("contextmenuaction(%i)", i - CONTEXT_BUTTON_USER1);
    if (pItem->GetProperty(action).empty())
      break;

    buttons.Add((CONTEXT_BUTTON)i, pItem->GetProperty(label).asString());
  }

  if (pItem->GetProperty("pluginreplacecontextitems").asBoolean())
    return;

  // TODO: FAVOURITES Conditions on masterlock and localisation
  if (!pItem->IsParentFolder() && !pItem->GetPath().Equals("add") && !pItem->GetPath().Equals("newplaylist://") &&
      !pItem->GetPath().Left(19).Equals("newsmartplaylist://"))
  {
...
bool CGUIJukeboxWindow::OnContextButton(CFileItemPtr pItem, CONTEXT_BUTTON button)
{
  switch (button)
  {
  ...
  case CONTEXT_BUTTON_USER10:
  {
    ...
    g_application.getApplicationMessenger().ExecBuiltIn(pItem->GetProperty(action).asString());
    return true;
  }
  default:
    break;
...
const CGUIViewState *CGUIJukeboxWindow::GetViewState() const
{
  return m_guiState.get();
}

const CFileItemList& CGUIJukeboxWindow::CurrentDirectory() const
{
  return *m_vecItems;
}

bool CGUIJukeboxWindow::WaitForNetwork() const
{
...
void CGUIJukeboxWindow::OnFilterItems(const CStdString &filter)
{
  ...
  m_albunsViewControl.SetItems(*m_vecItems);
  m_albunsViewControl.SetSelectedItem(currentItem);
  UpdateButtons();
}

void CGUIJukeboxWindow::GetFilteredItems(const CStdString &filter, CFileItemList &items)
{
...
CStdString CGUIJukeboxWindow::GetStartFolder(const CStdString &dir)
{
