...
#ifdef IS_JUKEBOX
  m_viewTracks.Reset();
  m_viewTracks.SetParentWindow(GetID());

  CGUIControl *control = (CGUIControl *)GetControl(CONTROL_JUKEBOXLIST);
  if (control && control->IsContainer())
    m_viewTracks.AddView(control);
  m_viewTracks.SetViewControlID(CONTROL_JUKEBOXLIST);
#endif
...
bool CGUIMediaWindow::OnAction(const CAction &action)
{
  ...
  if (action.GetID() == ACTION_CONTEXT_MENU && !m_viewControl.HasControl(GetFocusedControlID()))
  {
    ...
    OnPopupMenu(m_vecItems->Get(m_viewControl.GetSelectedItem()));
    return true;
  }
  ...
  case GUI_MSG_CLICKED:
    {
...
#ifdef IS_JUKEBOX // Laureon: Added: Jukebox Media listing behavior 
      else if (iControl == CONTROL_JUKEBOXLIST) {
        CGUIListContainer *control = (CGUIListContainer *)GetControl(CONTROL_JUKEBOXLIST);

        if (control && control->IsContainer()) {
          int iAction = message.GetParam1();

          if (iAction == ACTION_SELECT_ITEM || iAction == ACTION_MOUSE_LEFT_CLICK)
            OnSelect(m_vecTracks->Get(control->GetSelectedItem()));
      ...
      else if (m_viewControl.HasControl(iControl)) { // Laureon: Its an album or anything else...
        int iAction = message.GetParam1();
        if (iAction == ACTION_SELECT_ITEM || iAction == ACTION_MOUSE_LEFT_CLICK)
        {
          OnSelect(m_vecItems->Get(m_viewControl.GetSelectedItem()));
        }
        if (iAction == ACTION_CONTEXT_MENU || iAction == ACTION_MOUSE_RIGHT_CLICK)
        {
          OnPopupMenu(m_vecItems->Get(m_viewControl.GetSelectedItem()));
...
bool CGUIMediaWindow::OnClick(CFileItemPtr pItem)
{
  ...
  if (pItem->GetPath() == "add" || pItem->GetPath() == "sources://add/") // 'add source button' in empty root
  {
    OnContextButton(0, CONTEXT_BUTTON_ADD_SOURCE); // Laureon: Check this. I think this is a control for something it was iItem in the first parameter
    return true;
  }
...
void CGUIMediaWindow::OnDeleteItem(CFileItemPtr pItem)
{
  if (!pItem) return;
...
void CGUIMediaWindow::GetContextButtons(CFileItemPtr pItem, CContextButtons &buttons)
{
  if (!pItem)
    return;
...
bool CGUIMediaWindow::OnContextButton(CFileItemPtr pItem, CONTEXT_BUTTON button)
{
  switch (button)
  {
  case CONTEXT_BUTTON_ADD_FAVOURITE:
