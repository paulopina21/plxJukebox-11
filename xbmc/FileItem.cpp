...
#ifndef IS_JUKEBOX // Laureon: IFNDEF: Like this the jukebox system will only respond true for video extension files...
  if (HasVideoInfoTag()) return true;
  if (HasMusicInfoTag()) return false;
  if (HasPictureInfoTag()) return false;
#endif // Laureon: ASK: This is really needed now??
...
bool CFileItem::IsFileFolder() const
{
  return (
   ...
   (IsPlayList() && g_advancedSettings.m_playlistAsFolders) ||
    IsZIP() ||
//    IsPFC() || // Laureon: Added: Filesystem
    IsRAR() ||
    IsRSS() ||
    IsType(".ogg") ||
    IsType(".nsf") ||
...
bool CFileItem::IsPFC() const
{
  return URIUtils::IsPFC(m_strPath);
}
...
void CFileItem::FillInDefaultIcon()
{
  ...
  if (!HasOverlay())
  {
    if (URIUtils::IsInRAR(m_strPath))
      ...
      SetOverlayImage(CGUIListItem::ICON_OVERLAY_AUDIO); // Laureon: Added: Audio overlay Icon
    else if (IsVideo())
      SetOverlayImage(CGUIListItem::ICON_OVERLAY_VIDEO); // Laureon: Added: Video overlay Icon
    else if (IsVideoDb())
      SetOverlayImage(CGUIListItem::ICON_OVERLAY_VIDEO); // Laureon: Added: Video DB overlay Icon. TODO: Change this to specific overlay
    ...
    if (item->m_bIsFolder)
    {
      ...
      if( !item->IsRemote()
        ...
        || URIUtils::IsInZIP(item->GetPath())
      	|| URIUtils::IsInPFC(item->GetPath()) // Laureon: Added: Filesystem
...
CStdString CFileItem::GetUserMusicThumb(bool alwaysCheckRemote /* = false */) const
{
  ...
  else if (!IsRemote() || alwaysCheckRemote /*|| g_guiSettings.GetBool("musicfiles.findremotethumbs")*/) // Laureon: Commented
  {
...
CStdString CFileItem::GetTBNFile() const
{
  ...
  if (URIUtils::IsInRAR(strFile) || URIUtils::IsInZIP(strFile) || URIUtils::IsInPFC(strFile)) // Laureon: Added: Filesystem
  {
...
CStdString CFileItem::GetFolderThumb(const CStdString &folderJPG /* = "folder.jpg" */) const
{
  ...
  if (IsStack() ||
      ...
      URIUtils::IsInPFC(strFolder) || // Laureon: Added: Filesystem
...
{
  if (IsPFC()) { // Laureon: PFC Fanart System
    CStdStringArray fanarts;
    StringUtils::SplitString(g_advancedSettings.m_fanartImages, "|", fanarts);
    CStdString strFullFanartPath;

    for (unsigned int i = 0; i < fanarts.size(); ++i) {
      URIUtils::AddFileToFolder(m_strPath,fanarts[i],strFullFanartPath);
//      URIUtils::CreateArchivePath(strFullFanartPath,"pfc", m_strPath, fanarts[i]);

      if (CFile::Exists(strFullFanartPath))
        return strFullFanartPath;
    }
  }
