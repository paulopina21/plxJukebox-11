...
bool CXRandR::SetMode(XOutput output, XMode mode)
{
  ...
  char cmd[255];
  if (getenv("RAVEN_BIN_HOME"))
    snprintf(cmd, sizeof(cmd), "%s/xrandr --output %s --mode %s", getenv("RAVEN_BIN_HOME"), outputFound.name.c_str(), modeFound.id.c_str());
  else
    return false;
...
void CXRandR::LoadCustomModeLinesToAllOutputs(void)
{
  ...
  if (!xmlDoc.LoadFile("special://root/userdata/ModeLines.xml"))
  {
    return;
  }
  ...
  for (TiXmlElement* modeline = pRootElement->FirstChildElement("modeline"); modeline; modeline = modeline->NextSiblingElement("modeline"))
  {
    ...
    if (getenv("RAVEN_BIN_HOME"))
    {
      snprintf(cmd, sizeof(cmd), "%s/xrandr --newmode \"%s\" %s > /dev/null 2>&1", getenv("RAVEN_BIN_HOME"),
    ...
    for (unsigned int i = 0; i < m_outputs.size(); i++)
    {
      if (getenv("RAVEN_BIN_HOME"))
      {
        snprintf(cmd, sizeof(cmd), "%s/xrandr --addmode %s \"%s\"  > /dev/null 2>&1", getenv("RAVEN_BIN_HOME"),
