...
bool URIUtils::ProtocolHasParentInHostname(const CStdString& prot)
{
  return prot.Equals("zip")
      || prot.Equals("rar")
      || prot.Equals("pfc"); // Laureon: Added: Filesystem
}
...
bool URIUtils::IsOnLAN(const CStdString& strPath)
{
  ...
  CURL url(strPath);
  if (url.GetProtocol() == "rar" || url.GetProtocol() == "zip" || url.GetProtocol() == "pfc") // Laureon: Added: Filesystem
...
bool URIUtils::IsPFC(const CStdString& strFile) {
  if (IsURL(strFile)) {
    CURL urlPath(strFile);

    if (urlPath.GetFileName().IsEmpty())
      return urlPath.GetProtocol() == "pfc";
  }

  CStdString strExtension; 
  GetExtension(strFile,strExtension);

  if (strExtension.CompareNoCase(".pfc") == 0)
    return true;
...
{
  return IsInZIP(strFile) || IsInRAR(strFile) || IsInPFC(strFile); // Laureon: Added: Filesystem
}

bool URIUtils::IsInPFC(const CStdString& strFile)
{
  CURL url(strFile);

  return url.GetProtocol() == "pfc" && url.GetFileName() != "";
}
