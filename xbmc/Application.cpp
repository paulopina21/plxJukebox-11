...
#ifdef IS_JUKEBOX
#include "music/windows/GUIWindowJukeboxSongs.h"
#include "music/windows/GUIWindowJukeboxNav.h"
#else
...
void CApplication::FatalErrorHandler(bool WindowSystemInitialized, bool MapDrives, bool InitNetwork)
{
  ...
  fprintf(stderr, "Error log at %sraven.log\n", g_settings.m_logFolder.c_str());
  abort();
}
...
static void CopyUserDataIfNeeded(const CStdString &strPath, const CStdString &file)
{
  ...
  if (!CFile::Exists(destPath))
  {
    ...
    CStdString srcPath = URIUtils::AddFileToFolder("special://root/userdata/", file);
...
void CApplication::Preflight()
{
  ...
  setenv("RAVEN_HOME", install_path.c_str(), 0);
...
bool CApplication::Create()
{
  ...
  g_application.EnablePlatformDirectories(false); // Laureon: Always in portable mode!!
  ...
  {
    fprintf(stderr,"Could not init logging classes. Permission errors on ~/.raven (%s)\n",
  ...
  CLog::Log(LOGNOTICE, "Starting Raven (%s), Platform: Darwin OSX (%s). Built on %s", g_infoManager.GetVersion().c_str(), g_sysinfo.GetUnameVersion().c_str(), __DATE__);
#elif defined(TARGET_DARWIN_IOS)
  CLog::Log(LOGNOTICE, "Starting Raven (%s), Platform: Darwin iOS (%s). Built on %s", g_infoManager.GetVersion().c_str(), g_sysinfo.GetUnameVersion().c_str(), __DATE__);
#elif defined(__FreeBSD__)
  CLog::Log(LOGNOTICE, "Starting Raven (%s), Platform: FreeBSD (%s). Built on %s", g_infoManager.GetVersion().c_str(), g_sysinfo.GetUnameVersion().c_str(), __DATE__);
#elif defined(_LINUX)
  CLog::Log(LOGNOTICE, "Starting Raven (%s), Platform: Linux (%s, %s). Built on %s", g_infoManager.GetVersion().c_str(), g_sysinfo.GetLinuxDistro().c_str(), g_sysinfo.GetUnameVersion().c_str(), __DATE__);
#elif defined(_WIN32)
  CLog::Log(LOGNOTICE, "Starting Raven (%s), Platform: %s. Built on %s (compiler %i)", g_infoManager.GetVersion().c_str(), g_sysinfo.GetKernelVersion().c_str(), __DATE__, _MSC_VER);
  ...
  CLog::Log(LOGNOTICE, "Log File is located: %sraven.log", g_settings.m_logFolder.c_str());
  CLog::Log(LOGNOTICE, "-----------------------------------------------------------------------");
  ...
  if (URIUtils::IsDVD(strExecutablePath))
  {
    ...
    {
      CFileItemList items;
      CUtil::GetRecursiveListing("special://root/userdata",items,"");
  ...
  strLangInfoPath.Format("special://root/language/%s/langinfo.xml", strLanguage.c_str());

  CLog::Log(LOGINFO, "load language info file: %s", strLangInfoPath.c_str());
  g_langInfo.Load(strLangInfoPath);

  CStdString strLanguagePath;
  strLanguagePath.Format("special://root/language/%s/strings.xml", strLanguage.c_str());
...
#ifdef _DEBUG // Laureon: Added: no fullscreen debug 
  ...
  if (!g_Windowing.CreateNewWindow("Raven", bFullScreen, g_settings.m_ResInfo[RES_WINDOW], OnEvent))
  {
  ...
  if (!g_Windowing.CreateNewWindow("Raven", bFullScreen, g_settings.m_ResInfo[g_guiSettings.m_LookAndFeelResolution], OnEvent))
  {
    CLog::Log(LOGFATAL, "CApplication::Create: Unable to create window");
    return false;
  }
  ...
  if (g_advancedSettings.m_splashImage)
  {
    ...
    {
      CLog::Log(LOGINFO, "load default splash image: %s", CSpecialProtocol::TranslatePath("special://root/media/Splash.png").c_str());
      m_splash = new CSplash("special://root/media/Splash.png");
    }
    m_splash->Show();
  }
...
/*
   ...
   special://root/          => [read-only] system directory (/usr/share/xbmc)
   special://home/          => [read-write] user's directory that will override special://root/ system-wide
                               installations like skins, screensavers, etc.
                               ($HOME/.raven)
                               NOTE: XBMC will look in both special://root/addons and special://home/addons for addons.
...
*/
  ...
  CUtil::GetHomePath(xbmcBinPath, "RAVEN_BIN_HOME");
  xbmcPath = getenv("RAVEN_HOME");
  ...
  if (xbmcPath.IsEmpty())
  {
    ...
    if (!CFile::Exists(URIUtils::AddFileToFolder(xbmcPath, "language")))
    {
      ...
      if (!CFile::Exists(URIUtils::AddFileToFolder(xbmcPath, "language")))
      {
        fprintf(stderr, "Unable to find path to Raven data files!\n");
        exit(1);
      }
    }
  }
  ...
  setenv("RAVEN_BIN_HOME", xbmcBinPath.c_str(), 0);
  setenv("RAVEN_HOME", xbmcPath.c_str(), 0);
  ...
  if (m_bPlatformDirectories)
  {
    ...
    CSpecialProtocol::SetHomePath(userHome + "/.raven");
    CSpecialProtocol::SetMasterProfilePath(userHome + "/.raven/userdata");

    CStdString strTempPath = URIUtils::AddFileToFolder(userHome, ".raven/temp");
  ...
  else
  {
    ...
    CSpecialProtocol::SetXBMCBinPath(xbmcBinPath);
    CSpecialProtocol::SetXBMCPath(xbmcPath);
    CSpecialProtocol::SetHomePath(URIUtils::AddFileToFolder(xbmcPath, "appdata"));
    CSpecialProtocol::SetMasterProfilePath(URIUtils::AddFileToFolder(xbmcPath, "appdata/userdata"));

    CStdString strTempPath = URIUtils::AddFileToFolder(xbmcPath, "appdata/temp");
    CSpecialProtocol::SetTempPath(strTempPath);
    CreateUserDirs();
...
bool CApplication::InitDirectoriesOSX()
{
  ...
  if (m_bPlatformDirectories)
  {
    ...
    CSpecialProtocol::SetXBMCPath(xbmcPath);
    #if defined(__arm__)
      CSpecialProtocol::SetHomePath(userHome + "/Library/Preferences/raven");
      CSpecialProtocol::SetMasterProfilePath(userHome + "/Library/Preferences/raven/userdata");
    #else
      CSpecialProtocol::SetHomePath(userHome + "/Library/Application Support/raven");
      CSpecialProtocol::SetMasterProfilePath(userHome + "/Library/Application Support/raven/userdata");
    #endif

    // location for temp files
    #if defined(__arm__)
      CStdString strTempPath = URIUtils::AddFileToFolder(userHome,  "Library/Preferences/raven/temp");
    #else
      CStdString strTempPath = URIUtils::AddFileToFolder(userHome, ".raven/");
      CDirectory::Create(strTempPath);
      strTempPath = URIUtils::AddFileToFolder(userHome, ".raven/temp");
    #endif
    CSpecialProtocol::SetTempPath(strTempPath);
...
bool CApplication::InitDirectoriesWin32()
{
  ...
  CUtil::GetHomePath(xbmcPath);
  SetEnvironmentVariable("RAVEN_HOME", xbmcPath.c_str());
  CSpecialProtocol::SetXBMCBinPath(xbmcPath);
  CSpecialProtocol::SetXBMCPath(xbmcPath);
...
#endif
  {
    CDirectory::Create("special://root/language");
    CDirectory::Create("special://root/addons");
    CDirectory::Create("special://root/sounds");
  }
...
#ifndef IS_PROFESSIONAL //Laureon: Added: Removing some windows
...
#ifdef IS_JUKEBOX
  g_windowManager.Add(new CGUIWindowJukeboxSongs);             // window id = 501
  g_windowManager.Add(new CGUIWindowJukeboxNav);               // window id = 502
#else
...
#ifdef IS_JUKEBOX // Laureon: Added: Log Events
  CLog::Log(LOGNOTICE, "-----------------------------------------------------------------------");
#ifdef IS_PROFESSIONAL
  CLog::Log(LOGNOTICE, "  JUKEBOX PROFESSIONAL!!!");

  CLog::Log(LOGNOTICE, "  STARTING JUKEBOX MANAGERS...");
  if (g_jukeboxManager.Start())
    CLog::Log(LOGNOTICE, "  WELL DONE!!");
  else {
    CLog::Log(LOGERROR, "  OOps... Something is wrong here...");
    return false;
  }
#else
  CLog::Log(LOGNOTICE, "  JUKEBOX HOME EDITION!!");
#endif
  CLog::Log(LOGNOTICE, "-----------------------------------------------------------------------");
#endif
...
bool CApplication::OnKey(const CKey& key)
{
  ...
  ResetScreenSaver();
  g_jukeboxManager.GetRandomManager().Reset(); // Laureon: SmartRandom Reset On key press...
...
bool CApplication::Cleanup()
{
  try
  {
    ...
    g_windowManager.Remove(WINDOW_SETTINGS_JUKEBOX_MANAGER);
    g_windowManager.Remove(WINDOW_SETTINGS_JUKEBOX_OPERATOR);
...
#endif
  //Laureon: Always save the volume
  g_settings.Save();
}
