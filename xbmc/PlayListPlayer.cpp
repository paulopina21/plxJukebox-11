...
int CPlayListPlayer::GetNextSong()
{
  ...
  if (m_iCurrentSong == 0)
    Remove(m_iCurrentPlayList, m_iCurrentSong);
  else
    Remove(m_iCurrentPlayList, m_iCurrentSong -1);
...
//  if (iSong >0) {
//    Remove(m_iCurrentPlayList,iSong - 1);
//  }
//  else
//    Remove(m_iCurrentPlayList, iSong);
...
void CPlayListPlayer::Add(int iPlaylist, CPlayList& playlist)
{
  ...
  // Laureon: its likely that the playlist changed
  CGUIMessage msg(GUI_MSG_PLAYLIST_CHANGED, 0, 0);
  g_windowManager.SendMessage(msg);
}
