...
bool CFile::Cache(const CStdString& strFileName, const CStdString& strDest, XFILE::IFileCallback* pCallback, void* pContext)
{
  ...
  if (URIUtils::IsInZIP(strFileName) || URIUtils::IsInPFC(strFileName)) // Laureon: Added: Filesystem: Special case PFC [Ignore caching]
...
bool CFile::Open(const CStdString& strFileName, unsigned int flags)
{
  ...
  try
  {
    ...
    if (url2.GetProtocol() == "pfc") // Laureon: Added: Filesystem
    	url2.SetOptions("");
