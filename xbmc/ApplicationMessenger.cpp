    ...
    case TMSG_PICTURE_SHOW:
      {
        ...
        if (URIUtils::IsZIP(pMsg->strParam) || URIUtils::IsRAR(pMsg->strParam) || URIUtils::IsPFC(pMsg->strParam)) // actually a cbz/cbr
        {// Laureon: Added: Filesystem
          CFileItemList items;
          CStdString strPath;
          if (URIUtils::IsZIP(pMsg->strParam))
            URIUtils::CreateArchivePath(strPath, "zip", pMsg->strParam.c_str(), "");
          else if (!URIUtils::IsPFC(pMsg->strParam)) // Laureon: Added: Filesystem
            URIUtils::CreateArchivePath(strPath, "rar", pMsg->strParam.c_str(), "");
          else
          	URIUtils::CreateArchivePath(strPath, "pfc", pMsg->strParam.c_str(), "");
