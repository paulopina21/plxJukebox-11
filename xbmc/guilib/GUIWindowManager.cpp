...
void CGUIWindowManager::Add(CGUIWindow* pWindow)
{
  ...
  int iWindowID = pWindow->GetID();

  if (iWindowID == WINDOW_SETTINGS_SYSTEM) {
    CLog::Log(LOGINFO, "LAUREON: LOADED %u to the window manager", iWindowID);
  }
  // push back all the windows if there are more than one covered by this class
  CSingleLock lock(g_graphicsContext);
  for (int i = 0; i < pWindow->GetIDRange(); i++)
  {
    WindowMap::iterator it = m_mapWindows.find(iWindowID + i);
    if (it != m_mapWindows.end())
    {
      CLog::Log(LOGERROR, "Error, trying to add a second window with id %u to the window manager", iWindowID);
      return;
    }
    m_mapWindows.insert(pair<int, CGUIWindow *>(iWindowID + i, pWindow));
  }
}
