...
const infomap system_labels[] =  {{ "hasnetwork",       SYSTEM_ETHERNET_LINK_ACTIVE },
                                  ...
                                  {"isprofessional",    SYSTEM_ISPROFESSIONAL},
                                  {"ishomeedition", 		SYSTEM_ISHOMEEDITION},
...
CStdString CGUIInfoManager::GetLabel(int info, int contextWindow)
{
  ...
  {
#ifdef IS_PROFESSIONAL // Laureon: Added: Jukebox Credits System
  case PLAYER_MODEINFO:
    strLabel = g_jukeboxManager.GetModeManager().GetModeInfo();
    break;
#endif
...
bool CGUIInfoManager::GetBool(int condition1, int contextWindow, const CGUIListItem *item)
{
  ...
  else if (condition == SYSTEM_ISPROFESSIONAL)
#ifdef IS_PROFESSIONAL // Laureon: Added: Jukebox Credits System
    bReturn = true;
#else
    bReturn = false;
#endif
    else if (condition == SYSTEM_ISHOMEEDITION)
#ifdef IS_HOMEEDITION // Laureon: Added: Jukebox Home Edition
    bReturn = true;
#else
    bReturn = false;
#endif
    ...
    case MUSICPLAYER_PLAYLISTPLAYING:
      {
        bReturn = false;
#ifdef IS_JUKEBOX
        if (g_application.IsPlaying() && g_playlistPlayer.GetCurrentPlaylist() == PLAYLIST_MUSIC)
          bReturn = true;        
#else
...
{
  CStdString tmp; // Laureon: Removed Git Rev display
//#ifdef GIT_REV 
...
CStdString CGUIInfoManager::GetItemLabel(const CFileItem *item, int info)
{
  ...
  case LISTITEM_TYPEOVERLAY: // Laureon: Added: Jukebox Media Type Overlay Icon
    return item->GetOverlayImage();
  case LISTITEM_THUMB:
