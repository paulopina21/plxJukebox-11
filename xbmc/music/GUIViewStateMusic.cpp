...
{
#ifndef IS_PROFESSIONAL // Laureon: Added: Jukebox: Professional version shouldn't list addons sources
...
#ifdef IS_JUKEBOX // Laureon: Added: Jukebox file listing sorting
    {
      if (g_guiSettings.GetBool("filelists.ignorethewhensorting"))
      {
        AddSortMethod(SORT_METHOD_ARTIST_IGNORE_THE, 551, LABEL_MASKS("%F", "", strAlbumLeft, strAlbumRight));  // Filename, empty | Artist, empty
        SetSortMethod(SORT_METHOD_ARTIST_IGNORE_THE);
      }
      else
      {
        AddSortMethod(SORT_METHOD_ARTIST, 551, LABEL_MASKS("%F", "", strAlbumLeft, strAlbumRight));  // Filename, empty | Artist, empty
        SetSortMethod(SORT_METHOD_ARTIST);
      }
