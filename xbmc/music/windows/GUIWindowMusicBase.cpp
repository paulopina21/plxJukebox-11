...
void CGUIWindowMusicBase::OnQueueItem(int iItem)
{
  ...
  if (item->IsRAR() || item->IsZIP() || item->IsPFC()) // Laureon: Added: Filesystem
    return;
...
//      CGUIDialogKaiToast::QueueNotification(, g_localizeStrings.Get(20052),g_localizeStrings.Get(70000);
    g_jukeboxManager.GetModeManager().ShowMessage();
//    if (!m_dlgInsertCoin->IsDialogRunning())
//        m_dlgInsertCoin->QueueNotification(CGUIDialogKaiToast::Warning, g_localizeStrings.Get(20052),g_localizeStrings.Get(70000),3000);
