...
/* We don't want to show Autosourced items (ie removable pendrives, memorycards) in Library mode */
...
#ifdef IS_JUKEBOX // Laureon: Added: Jukebox Music library behavior
      ...
      if (control && control->IsContainer()) {
        CFileItemPtr selectedItem = m_vecItems->Get(m_viewControl.GetSelectedItem()); 
        GetDirectory(selectedItem->GetPath(), *m_vecTracks);

        if (selectedItem->IsAlbum()) // Laureon: Jukebox: Music Video System: Testing if an item is an album first to list its musics
          m_videodatabase.GetMusicVideosByAlbum(selectedItem->GetMusicInfoTag()->GetAlbum(), *m_vecTracks);
...
{
#ifdef IS_JUKEBOX   // Laureon: Added: Jukebox Listing System
  ...
  if (actionID == ACTION_MOVE_RIGHT || actionID == ACTION_MOVE_LEFT) {
    ...
    if (GetFocusedControlID() == m_viewControl.GetCurrentControl()) {
      CFileItemPtr selectedItem = m_vecItems->Get(m_viewControl.GetSelectedItem()); 
      GetDirectory(selectedItem->GetPath(), *m_vecTracks);

      if (selectedItem->IsAlbum()) { // Laureon: Jukebox: Music Video System: Testing if an item is an album first to list its music videos...
        if (m_videodatabase.Open()) {
          CStdString sqlWhere;
          sqlWhere.Format("where c09 like '%s' and c10 like '%s'" ,selectedItem->GetMusicInfoTag()->GetAlbum(), selectedItem->GetMusicInfoTag()->GetArtist());

          CFileItemList *videoList = new CFileItemList;
          m_videodatabase.GetMusicVideosByWhere("videodb://3/2/", sqlWhere, *videoList);
          m_vecTracks->Append(*videoList);
          delete videoList;
...
bool CGUIWindowMusicNav::OnClick(CFileItemPtr pItem)
{
  ...
  return CGUIWindowMusicBase::OnClick(pItem);
}
