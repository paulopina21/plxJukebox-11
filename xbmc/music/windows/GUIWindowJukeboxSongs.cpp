#include "system.h"
#include "GUIWindowJukeboxSongs.h"
#include "Util.h"
...
CGUIWindowJukeboxSongs::CGUIWindowJukeboxSongs(void): CGUIWindowJukeboxBase(WINDOW_MUSIC_FILES, "JukeboxSongs.xml") {
  m_vecItems->SetPath("?");
...
CGUIWindowJukeboxSongs::~CGUIWindowJukeboxSongs(void) {
}

bool CGUIWindowJukeboxSongs::OnMessage(CGUIMessage& message) {
...
bool CGUIWindowJukeboxSongs::OnAction(const CAction& action) {
  ...
  return CGUIWindowJukeboxBase::OnAction(action);
}
...
bool CGUIWindowJukeboxSongs::GetDirectory(const CStdString &strDirectory, CFileItemList &items) {
  if (!CGUIWindowJukeboxBase::GetDirectory(strDirectory, items))
    return false;
...
void CGUIWindowJukeboxSongs::OnPrepareFileItems(CFileItemList &items) {
  RetrieveMusicInfo();

  items.SetCachedMusicThumbs();
}

void CGUIWindowJukeboxSongs::UpdateButtons() {
...
void CGUIWindowJukeboxSongs::GetContextButtons(CFileItemPtr pItem, CContextButtons &buttons) {
  if (pItem) {
    ...
    if (m_vecItems->IsVirtualDirectoryRoot()) {
      ...
      CGUIDialogContextMenu::GetContextButtons("music", pItem, buttons);
#ifdef HAS_DVD_DRIVE
      // enable Rip CD an audio disc
      if (g_mediaManager.IsDiscInDrive() && pItem->IsCDDA()) {
      ...
      if (pItem->GetProperty("pluginreplacecontextitems").asBoolean())
        return;
      if (!pItem->IsPlayList()) {
        if (pItem->IsAudio() && !pItem->IsLastFM())
          buttons.Add(CONTEXT_BUTTON_SONG_INFO, 658); // Song Info
        else if (!pItem->IsParentFolder() && !pItem->IsLastFM() &&
                !pItem->GetPath().Left(3).Equals("new") && pItem->m_bIsFolder) {
#if 0
      ...
      if (!pItem->IsParentFolder() && !pItem->IsReadOnly()) {
    ...
    if (pScanDlg) {
      if (pScanDlg->IsScanning())
        buttons.Add(CONTEXT_BUTTON_STOP_SCANNING, 13353); // Stop Scanning
              ...
              !pItem->IsLastFM() &&
              !pItem->GetPath().Equals("add") && !pItem->IsParentFolder() &&
              !pItem->IsPlugin() &&
              (g_settings.GetCurrentProfile().canWriteDatabases() || g_passwordManager.bMasterUser)) {
...
void CGUIWindowJukeboxSongs::DeleteRemoveableMediaDirectoryCache() {
  CUtil::DeleteDirectoryCache("r-");
}

void CGUIWindowJukeboxSongs::PlayItem(CFileItemPtr pItem) {
...
bool CGUIWindowJukeboxSongs::Update(const CStdString &strDirectory) {
  if (m_thumbLoader.IsLoading())
    m_thumbLoader.StopThread();
...
CStdString CGUIWindowJukeboxSongs::GetStartFolder(const CStdString &dir) {
  SetupShares();
  VECSOURCES shares;
