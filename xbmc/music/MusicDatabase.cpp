...
bool CMusicDatabase::CreateTables()
{
  ...
  try
  {
    ...
    m_pDS->exec("CREATE TABLE album ( idAlbum integer primary key, idPFCFile integer, strAlbum varchar(256), idArtist integer, strExtraArtists text, idGenre integer, iYear integer, idThumb integer, GTIN varchar(13) default NULL, iVisible integer default 1)\n");  // Laureon: Added GTIN and iVisible
    ...
    CLog::Log(LOGINFO, "create PFCFile table");
    m_pDS->exec("CREATE TABLE pfcfile ( idPFCFile INTEGER PRIMARY KEY, idPath INTEGER, strFileName TEXT, strHash TEXT)\n");
    CLog::Log(LOGINFO, "create song table");
    m_pDS->exec("CREATE TABLE song ( idSong integer primary key, idAlbum integer, idArtist integer, strExtraArtists text, idGenre integer, strExtraGenres text, strTitle varchar(512), iTrack integer, iDuration integer, iYear integer, dwFileNameCRC text, strFileName text, iTimesPlayed integer, iStartOffset integer, iEndOffset integer, idThumb integer, lastplayed varchar(20) default NULL, rating char default '0', comment text, ISRC varchar(12) default NULL, iVisible integer default 1)\n"); // Laureon: Added ISRC and iVisible. Removed Brainz shit
...
//    m_pDS->exec("CREATE TABLE musicvideo ( idVideo integer primary key, idAlbum integer, idPath integer, idArtist integer, strExtraArtists text, idGenre integer, strExtraGenres text, strTitle varchar(512), iTrack integer, iDuration integer, iYear integer, dwFileNameCRC text, strFileName text, iTimesPlayed integer, iStartOffset integer, iEndOffset integer, idThumb integer, lastplayed varchar(20) default NULL, rating char default '0', ISRC varchar(12) default NULL, iVisible integer default 1)\n"); // Laureon: Added MusicVideo table
    ...
    CLog::Log(LOGINFO, "create pfcfile index");
    m_pDS->exec("CREATE INDEX idxPFCFile ON pfcfile(strFileName)");
...
//    m_pDS->exec("CREATE INDEX idxSong6 ON song(idPath)");
    ...
    // Trigger
    CLog::Log(LOGINFO, "Database -> creating trigger: albuminfo");
    ...
    // Trigger: Delete on album
    CLog::Log(LOGINFO, "Database -> creating trigger: albumfile");
    m_pDS->exec("CREATE TRIGGER tgrAlbumFile AFTER delete ON album FOR EACH ROW BEGIN delete FROM pfcfile WHERE idPFCFile=pfcfile.idPFCFile; END");

    // Trigger: Delete on album -> cascade to songs
    CLog::Log(LOGINFO, "Database -> creating trigger: albumsongs");
    m_pDS->exec("CREATE TRIGGER tgrAlbumSongs AFTER delete ON album FOR EACH ROW BEGIN delete FROM song WHERE idAlbum=song.idAlbum; END");

    // Trigger: Delete on path
    CLog::Log(LOGINFO, "Database -> creating trigger: path");
    m_pDS->exec("CREATE TRIGGER tgrPath AFTER delete ON path FOR EACH ROW BEGIN delete FROM pfcfile WHERE idPath=pfcfile.idPath; END");

    // Trigger: Delete on pfcfile
    CLog::Log(LOGINFO, "Database -> creating trigger: pfcfile");
    m_pDS->exec("CREATE TRIGGER tgrPFCFile AFTER delete ON pfcfile FOR EACH ROW BEGIN delete FROM album WHERE idPFCFile=album.idPFCFile; END");
...
void CMusicDatabase::CreateViews()
{
  ...
  strSQL = "create view albumview as "
                  "select album.idAlbum as idAlbum, album.idArtist as idArtist, strAlbum, strPath || strFileName as strFilePath,  "
                  "strExtraArtists, strExtraGenres, album.idGenre as idGenre, strArtist, strGenre, "
                  "album.iYear as iYear, strThumb, idAlbumInfo, strMoods, strStyles, strThemes, "
                  "strReview, strLabel, strType, strImage, iRating, album.GTIN, album.iVisible from album "
  ...
  strSQL = "create view songview as "
                  "select song.idSong as idSong, song.idArtist as idArtist, song.idAlbum as idAlbum, "
                  "strArtist, strAlbum, iTrack, strTitle, iDuration, "
                  "path.strPath || pfcfile.strFileName as strPath, song.strFileName, "
                  "song.strExtraArtists as strExtraArtists, "
                  "song.strExtraGenres as strExtraGenres, "
                  "song.iYear as iYear, "
                  "dwFileNameCRC, "
                  "iTimesPlayed, iStartOffset, iEndOffset, lastplayed, rating, comment, "
                  "song.idGenre as idGenre, strGenre, "
                  "strThumb, "
                  "iKaraNumber, iKaraDelay, strKaraEncoding, "
                  "song.ISRC, song.iVisible "
                  "from song "
                  "join album on song.idAlbum=album.idAlbum "
                  "join artist on song.idArtist=artist.idArtist "
                  "left outer join thumb on song.idThumb=thumb.idThumb "
                  "join pfcfile on album.idPFCFile=pfcfile.idPFCFile "
                  "join path on pfcfile.idPath=path.idPath "
                  "join genre on song.idGenre=genre.idGenre "
                  "left outer join karaokedata on song.idSong=karaokedata.idSong";
//  m_pDS->exec("create view songview as select song.idSong as idSong, "
//      "song.strExtraArtists as strExtraArtists, song.strExtraGenres as strExtraGenres, "
//      "strTitle, iTrack, iDuration, song.iYear as iYear, dwFileNameCRC, strFileName, "
//      "iTimesPlayed, iStartOffset, iEndOffset, lastplayed, rating, comment, "
//      "song.idAlbum as idAlbum, strAlbum, strPath, song.idArtist as idArtist, "
//      "strArtist, song.idGenre as idGenre, strGenre, strThumb, iKaraNumber, "
//      "iKaraDelay, strKaraEncoding, song.ISRC, song.iVisible from song "
//      "join album on song.idAlbum=album.idAlbum "
////      "join path on song.idPath=path.idPath "
//      "join artist on song.idArtist=artist.idArtist "
//      "join genre on song.idGenre=genre.idGenre "
//      "join thumb on song.idThumb=thumb.idThumb "
//      "left outer join karaokedata on song.idSong=karaokedata.idSong"); // Laureon: Added ISRC field and visibility control. Removed Brainz shit



  m_pDS->exec(strSQL);
  //  m_pDS->exec("create view albumview as select album.idAlbum as idAlbum, album.idPFCFile as idPFCFile, strAlbum, strExtraArtists, "
...
//              "strReview, strLabel, strType, strImage, iRating, album.GTIN, album.iVisible from album " // Laureon: Added GTIN Field
//              "left outer join artist on album.idArtist=artist.idArtist "
//              "left outer join genre on album.idGenre=genre.idGenre "
//              "left outer join thumb on album.idThumb=thumb.idThumb "
//              "left outer join albuminfo on album.idAlbum=albuminfo.idAlbum "// Laureon: Added Album Visibility
//              "left outer join pfcfile on album.idPFCfile=album.idPFCFile "
//              );
}
...
void CMusicDatabase::AddSong(CSong& song, bool bCheck)
{
  ...
  try
  {
    ...
    CLog::Log(LOGDEBUG, "%s(%s)", __FUNCTION__, song.strFileName.c_str());

    // Laureon: Album info must be inserted first now 'cause the album carries the PFCFileName now...
    int idAlbum = GetAlbumByName(song.strAlbum, song.strAlbumArtist);
    if (idAlbum == -1) {
      CLog::Log(LOGERROR,"AddSong: Couldn't find album: %s", song.strAlbum.c_str());
      return;
    }

    // split our (possibly) multiple artist string into individual artists
    // add the primary artist
...
//      idAlbum = AddAlbum(song.strAlbum, idArtist, extraArtists, song.strArtist, idThumb, idGenre, extraGenres, song.iYear);
    ...
    if (bInsert)
    {
      ...
      strSQL=PrepareSQL("insert into song (idSong,idAlbum,idArtist,strExtraArtists,idGenre,strExtraGenres,strTitle,iTrack,iDuration,iYear,dwFileNameCRC,strFileName,ISRC,iTimesPlayed,iStartOffset,iEndOffset,idThumb,lastplayed,rating,comment) values (NULL,%i,%i,'%s',%i,'%s','%s',%i,%i,%i,'%ul','%s','%s'",
                    idAlbum, idArtist, extraArtists.c_str(), idGenre, extraGenres.c_str(),
                    song.strTitle.c_str(),
    ...
    CLog::Log(LOGDEBUG, "%s -> Added. New song index: %i", __FUNCTION__, idSong);
  }
  catch (...)
  {
...
int CMusicDatabase::AddAlbum(CAlbum& album) {
  CStdString strSQL;
  try
  {
    ...
    CStdString strArtist = album.strArtist;
    CStdString strPFCFile = album.strPFCFile;
    int idThumb = 0;

    // Laureon: Some minor tweaks to ensure some minor things... :P
    strAlbum.TrimLeft(" ");
    strAlbum.TrimRight(" ");
    CLog::Log(LOGDEBUG, "%s(%s)", __FUNCTION__, strAlbum.c_str());
    if (strAlbum.IsEmpty())
    {
    ...
    // Laureon: Resolve file name things...
    CURL strURL(strPFCFile);
    CStdString strPath = strURL.GetWithoutFilename();
    CStdString strFileName = strURL.GetFileNameWithoutPath();
    URIUtils::GetDirectory(strPFCFile,strPath);

    int idPath = AddPath(strPath);
    if (idPath == -1) return -1;
    int idPFCFile = AddPFCFile(idPath, strFileName);
    if (idPFCFile == -1) return -1;

    // Laureon: Workaround various artists names
    CStdStringArray vecAlbumArtists; CStdString extraArtists;
    SplitString(strArtist, vecAlbumArtists, extraArtists);

    // Laureon: the same for our genres
    CStdStringArray vecGenres; CStdString extraGenres;
    SplitString(album.strGenre, vecGenres, extraGenres);

    // Laureon: These guys already check for existence...
    int idArtist  = AddArtist(vecAlbumArtists[0]);
    int idGenre = AddGenre(vecGenres[0]);

    album.idArtist = idArtist;
    album.idPFCFile = idPFCFile;
    album.strPFCFile = strPFCFile;

    map <CStdString, CAlbumCache>::const_iterator it = m_albumCache.find(strAlbum + strArtist);
    if (it != m_albumCache.end())
      return it->second.idAlbum;

    strSQL=PrepareSQL("SELECT idAlbum FROM album WHERE idArtist=%i AND strAlbum LIKE '%s'", idArtist, strAlbum.c_str());
    ...
    if (m_pDS->num_rows() == 0)
    {
      ...
      CLog::Log(LOGDEBUG, "%s -> Adding to album to database: %s)", __FUNCTION__, strAlbum.c_str());
      strSQL=PrepareSQL("INSERT INTO album (idAlbum, idPFCFile, strAlbum, idArtist, strExtraArtists, idGenre, iYear, idThumb, GTIN) VALUES( NULL, %i,'%s', %i, '%s', %i, %i, %i, '%s')", idPFCFile, strAlbum.c_str(), idArtist, extraArtists.c_str(), idGenre, album.iYear, idThumb, album.strGTIN.c_str());
      m_pDS->exec(strSQL.c_str());

      album.idAlbum = (int)m_pDS->lastinsertid();

      CLog::Log(LOGDEBUG, "%s -> Looking for songs: %s)", __FUNCTION__, strAlbum.c_str());
      for (VECSONGS::iterator itSong = album.songs.begin(); itSong != album.songs.end(); ++itSong)
      {
        itSong->iAlbumId = album.idAlbum;
        itSong->iArtistId   = idArtist;
        AddSong(*itSong, true);
      }

      CLog::Log(LOGDEBUG, "%s -> Added all songs I could found!!!", __FUNCTION__);
    }
    else
    {
      ...
      m_pDS->close();
      strSQL=PrepareSQL("UPDATE album SET idPFCFile=%i, strExtraArtists='%s', idGenre=%i, strExtraGenres='%s', iYear=%i, idThumb=%i where idAlbum=%i", idPFCFile, extraArtists.c_str(), idGenre, extraGenres.c_str(), album.iYear, idThumb, album.idAlbum);
      m_pDS->exec(strSQL.c_str());
    ...
    CAlbumCache cAlbum;
    cAlbum.idAlbum = album.idAlbum;
    cAlbum.strAlbum = album.strAlbum;
    cAlbum.idArtist = album.idArtist;
    cAlbum.strArtist = album.strArtist;
    cAlbum.strPFCFile = album.strPFCFile; // Laureon: I'm using full file path in cache to speed things up (in cost of memory of course)...
    m_albumCache.insert(pair<CStdString, CAlbumCache>(cAlbum.strAlbum + cAlbum.strArtist, cAlbum));
...
int CMusicDatabase::AddAlbum(const CStdString& strAlbum1, int idArtist, const CStdString &extraArtists, const CStdString &strArtist, int idThumb, int idGenre, const CStdString &extraGenres, int year, const CStdString strGTIN) // Laureon: Added GTIN)
{
  CStdString strSQL;
  try
  {
    ...
    if (m_pDS->num_rows() == 0)
    {
      m_pDS->close();
      // doesnt exists, add it
      strSQL=PrepareSQL("insert into album (idAlbum, strAlbum, idArtist, strExtraArtists, idGenre, strExtraGenres, iYear, idThumb, GTIN) values( NULL, '%s', %i, '%s', %i, '%s', %i, %i, '%s')", strAlbum.c_str(), idArtist, extraArtists.c_str(), idGenre, extraGenres.c_str(), year, idThumb, strGTIN.c_str());
      m_pDS->exec(strSQL.c_str());
...
int CMusicDatabase::AddPFCFile(int idPath, const CStdString& strFileName)
{
  CStdString strSQL;
  try
  {
...
//      return it->second;
    ...
    strSQL=PrepareSQL( "SELECT idPFCFile FROM pfcfile WHERE strFileName LIKE '%s'", strFileName.c_str());
    m_pDS->query(strSQL.c_str());
    if (m_pDS->num_rows() == 0)
    {
      m_pDS->close();
      // doesnt exists, add it
      strSQL=PrepareSQL("INSERT INTO pfcfile(idPath, strFileName) VALUES(%i, '%s' )", idPath, strFileName.c_str());
      m_pDS->exec(strSQL.c_str());
...
//      m_pathCache.insert(pair<CStdString, int>(strPath, idPath));
    ...
    {
//      int idPath = m_pDS->fv("idPFCFile").get_asInt();
//      m_pathCache.insert(pair<CStdString, int>(strPath, idPath));
      m_pDS->close();
      return m_pDS->fv("idPFCFile").get_asInt();
    }
  }
  catch (...)
  {
    CLog::Log(LOGERROR, "musicDB: unable to AddPFC (%s)", strSQL.c_str());
  }
...
CSong CMusicDatabase::GetSongFromDataset(bool bWithMusicDbPath/*=false*/)
{
  ...
  song.iEndOffset = m_pDS->fv(song_iEndOffset).get_asInt(); // Laureon: {Removed Brainz shit}
  ...
  song.strISRC = m_pDS->fv(song_strISRC).get_asString(); // Laureon: Jukebox System: ISRC Loading from dataset
  song.iVisible = m_pDS->fv(song_iVisible).get_asInt(); // Laureon: Added: Song Visibility
  ...
  if (!bWithMusicDbPath)
  {
    CStdString strExtension = "pfc";//= URIUtils::GetExtension(m_pDS->fv(song_strPath).get_asString());

//    if (strExtension.empty())
//      strExtension = "pfc";
    URIUtils::CreateArchivePath(song.strFileName, strExtension, m_pDS->fv(song_strPath).get_asString(), m_pDS->fv(song_strFileName).get_asString());
...
void CMusicDatabase::GetFileItemFromDataset(CFileItem* item, const CStdString& strMusicDBbasePath)
{
  ...
  item->m_lEndOffset = m_pDS->fv(song_iEndOffset).get_asInt(); // Laureon: Removed Brainz shit
  ...
  item->GetMusicInfoTag()->SetISRC(m_pDS->fv(song_strISRC).get_asString()); // Laureon: Jukebox System: ISRC Loading from dataset to MusicInfoTag

  // Laureon: Music file path workaround
  CStdString strRealPath;
  CStdString strExtension = "pfc";//URIUtils::GetExtension(m_pDS->fv(song_strPath).get_asString());
//  if (strExtension.empty())
//    strExtension = "pfc";
  URIUtils::CreateArchivePath(strRealPath, strExtension, m_pDS->fv(song_strPath).get_asString(), m_pDS->fv(song_strFileName).get_asString());
...
CAlbum CMusicDatabase::GetAlbumFromDataset(dbiplus::Dataset* pDS, bool imageURL /* = false*/)
{
  ...
  album.strGTIN = pDS->fv(album_GTIN).get_asString(); // Laureon: Added GTIN
  album.iVisible = pDS->fv(album_iVisible).get_asInt(); // Laureon: Added: Album Visibility
...
{
  CLog::Log(LOGDEBUG, "%s(%s)", __FUNCTION__, album.strAlbum.c_str());
  CStdString strSQL;
  try
  {
    if (NULL == m_pDB.get()) return -1;
...
bool CMusicDatabase::CleanupSongsByIds(const CStdString &strSongIds)
{
  try
  {
    ...
    CStdString strSQL=PrepareSQL("select idSong, strPath || pfcfile.strFileName as strPath, song.strFileName from song "
    "join album on song.idAlbum = album.idAlbum "
    "join pfcfile on pfcfile.idPFCFile = album.idPFCFile "
    "join path on path.idPath = pfcfile.idPath where song.idSong in %s", strSongIds.c_str());
...
//      URIUtils::AddFileToFolder(m_pDS->fv("strPath").get_asString(), m_pDS->fv("song.strFileName").get_asString(), strFileName);
      URIUtils::CreateArchivePath(strFileName, "pfc", m_pDS->fv("strPath").get_asString(), m_pDS->fv("song.strFileName").get_asString());
...
bool CMusicDatabase::GetAlbumsNav(const CStdString& strBaseDir, CFileItemList& items, int idGenre, int idArtist, int start, int end)
{
  ...
  {
    if (strWhere.IsEmpty())
      strWhere += "where iVisible = 1 and ";
    else
      strWhere += "and iVisible = 1 and ";
...
bool CMusicDatabase::GetAllAlbumsNav(const CStdString& strBaseDir, CFileItemList& items, int idGenre, int idArtist, int start, int end)
{
  //Create limit
  CStdString limit;
  if (start >= 0 && end >= 0)
...
bool CMusicDatabase::GetSongsNav(const CStdString& strBaseDir, CFileItemList& items, int idGenre, int idArtist,int idAlbum)
{
  ...
  strWhere += " ORDER BY iTrack"; // Laureon: Order tracks by track number
  // run query
...
bool CMusicDatabase::GetArtistPath(int idArtist, CStdString &basePath)
{
  try
  {
    ...
    // find all albums from this artist, and all the paths to the songs from those albums // Laureon: Path mess here!
    CStdString strSQL=PrepareSQL("select strPath || strFileName as strPath from album "
                                                          "join pfcfile on album.idPFCFile = pfcfile.idPFCFile "
                                                          "join path on pfcfile.idPath = path.idPath "
                                                          ...
                                                          "group by album.idPFCFile LIMIT 1", idArtist, idArtist);
    ...
    // special case for single path - assume that we're in an artist/album/songs filesystem // Laureon: Not anymore...
    if (iRowsFound > 0)
    {
      //URIUtils::GetParentPath(m_pDS2->fv("strPath").get_asString(), basePath);
      URIUtils::CreateArchivePath(basePath,"pfc",m_pDS2->fv("strPath").get_asString(),"");
...
int CMusicDatabase::GetSongIDFromPath(const CStdString &filePath) // Laureon: Modded this shit
{
  // grab the where string to identify the song id
  CURL url(filePath);
  CStdString strFilePath = url.GetHostName();
  if (url.GetProtocol()=="musicdb")
  {
  ...
  try
  {
    if (NULL == m_pDB.get()) return -1;
    if (NULL == m_pDS.get()) return -1;
    CStdString strFileName = URIUtils::GetFileName(filePath);
    //URIUtils::GetDirectory(filePath, strPath);
    URIUtils::AddSlashAtEnd(strFilePath);
    strFilePath += strFileName;

    DWORD crc = ComputeCRC(strFileName);

    CStdString sql = PrepareSQL("SELECT idSong FROM song "
        "JOIN album ON song.idAlbum = album.idAlbum "
        "JOIN pfcfile ON album.idPFCFile = pfcfile.idPFCFile "
        "WHERE song.dwFileNameCRC = '%ul' AND song.strFileName = '%s'", crc, strFileName.c_str());
  ...
  {
    CLog::Log(LOGERROR, "%s (%s) failed", __FUNCTION__, strFilePath.c_str());
  }
  return -1;
}
...
#ifdef IS_JUKEBOX
bool CMusicDatabase::SetAlbumVisible( int idAlbum, bool bVisible /*= true*/ ) {
  try 
  {
    if (NULL == m_pDB.get()) return 0;
    if (NULL == m_pDS.get()) return 0;
    ...
    (bVisible)? iVisible = 1 : iVisible = 0;

    CStdString strSQL = PrepareSQL("UPDATE album SET iVisible = %i WHERE idAlbum = %i", iVisible, idAlbum);
    CLog::Log(LOGDEBUG, "%s query = %s", __FUNCTION__, strSQL.c_str());
...
bool CMusicDatabase::SetSongVisible( int idSong, bool bVisible /*= true*/ ) {
  try 
  {
    if (NULL == m_pDB.get()) return 0;
    if (NULL == m_pDS.get()) return 0;
    ...
    CStdString strSQL = PrepareSQL("UPDATE song SET iVisible = %i WHERE idSong = %i", iVisible, idSong);
    CLog::Log(LOGDEBUG, "%s query = %s", __FUNCTION__, strSQL.c_str());
