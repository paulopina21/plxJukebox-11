...
//    return true;
  ...
  // load subfolder
  CFileItemList items;
//  CDirectory::GetDirectory(strDirectory, items, g_settings.m_musicExtensions +  g_settings.m_videoExtensions  +"|.jpg|.tbn|.lrc|.cdg");
  CDirectory::GetDirectory(strDirectory, items, ".pfc");
  ...
  items.SetMusicThumb(false); // true forces it to get a remote thumb
...
//    items.Sort(SORT_METHOD_LABEL, SORT_ORDER_ASC);
  ...
  for (int i = 0; i < items.Size(); ++i)
  {
    ...
    if (pItem->m_bIsFolder && !pItem->IsParentFolder() && !pItem->IsPlayList() && !pItem->IsPFC())
    {
...
//Get all albums for all PFCs
//Loop Albums
  //Generate Thumb and grab Thumb path
  //Update Thumb path from album.
...
bool CMusicInfoScanner::GetPFCAlbumInfo(CFileItemPtr pItem, CAlbum &album) {
  if (!pItem.get()->IsPFC()) {
    CLog::Log(LOGERROR,"%s -> This is not a PFC: %s", __FUNCTION__, pItem->GetPath().c_str());
    return false;
  }

  CStdString strNfo;
  CStdString strPath = pItem->GetPath();
  URIUtils::CreateArchivePath(strNfo, "pfc", strPath, "album.nfo");

  if ( !XFILE::CFile::Exists(strNfo) ) {
    CLog::Log(LOGERROR,"%s -> NFO not found for: %s", __FUNCTION__, strPath.c_str());
    return false;
  }

  CLog::Log(LOGDEBUG,"%s -> Found matching nfo file: %s", __FUNCTION__, strNfo.c_str());

  CNfoFile::NFOResult result=CNfoFile::NO_NFO;
  CNfoFile nfoReader;
  result = nfoReader.Create(strNfo, ADDON::ADDON_SCRAPER_ALBUMS, -1, strPath);

  if (result != CNfoFile::FULL_NFO) {
    CLog::Log(LOGERROR,"%s -> Couldn't retrieve full NFO for: %s", __FUNCTION__, strPath.c_str());
    return false;
  }

  if ( !nfoReader.GetDetails(album) ) {
    CLog::Log(LOGERROR,"%s -> Couldn't retrieve details for: %s", __FUNCTION__, strPath.c_str());
    return false;
  }

  CLog::Log(LOGDEBUG, "%s: Got details from nfo for album: %s", __FUNCTION__, strPath.c_str());
  album.strPFCFile = strPath;

  // Laureon: Duration Check...
  for (VECSONGS::iterator itSong = album.songs.begin(); itSong != album.songs.end(); ++itSong)
  {
    if (itSong->iDuration <= 0)
    {
      CStdString strFullSongPath;
      URIUtils::CreateArchivePath(strFullSongPath, "pfc", strPath, itSong->strFileName); // Laureon: Hack to insert file path in PFC songs...
      itSong->iDuration = GetSongDuration(strFullSongPath);
    }
  }
...
bool CMusicInfoScanner::GetPFCArtistInfo(CFileItemPtr pItem, CArtist &artist) {
  if (!pItem.get()->IsPFC()) {
    CLog::Log(LOGERROR,"%s -> This is not a PFC: %s", __FUNCTION__, pItem->GetPath().c_str());
    return false;
  }
  ...
  CStdString strNfo;
  CStdString strPath = pItem->GetPath();
  URIUtils::CreateArchivePath(strNfo, "pfc", strPath, "artist.nfo");
  ...
  CNfoFile::NFOResult result=CNfoFile::NO_NFO;
  CNfoFile nfoReader;
  result = nfoReader.Create(strNfo, ADDON::ADDON_SCRAPER_ARTISTS, -1, strPath);
  ...
  if ( !nfoReader.GetDetails(artist) ) {
    CLog::Log(LOGERROR,"%s -> Couldn't retrieve details for: %s", __FUNCTION__, strPath.c_str());
    return false;
  }
...
bool CMusicInfoScanner::GetPFCArtistArtwork(CFileItemPtr pItem) {
  CStdString strPath = pItem->GetPath();
  CStdString thumb =  pItem->GetCachedArtistThumb();

  if (!XFILE::CFile::Exists(thumb))
  {
    CStdString strLocalThumb = URIUtils::AddFileToFolder(strPath, "artist.jpg");
    if (XFILE::CFile::Exists(strLocalThumb))
      CPicture::CreateThumbnail(strLocalThumb, thumb);
  }

  // check fanart
  CStdString cachedImage = pItem->GetCachedFanart();
  if (!CFile::Exists(cachedImage))
  { // check for local fanart
    CLog::Log(LOGDEBUG, "%s -> looking for fanart for: %s", __FUNCTION__, strPath.c_str());
    if (!pItem->CacheLocalFanart())
    {
      CLog::Log(LOGERROR, "%s -> No fanart found for: %s", __FUNCTION__, strPath.c_str());
      return false;
    }
  }
  return true;
...
int CMusicInfoScanner::RetrievePFCSongs(CFileItemPtr pItem, VECSONGS &songs) {
  if (!pItem.get()->IsPFC()) return 0;

  CArtist artist;
  if (! GetPFCArtistInfo(pItem, artist) ) return -1;

  CAlbum album;
  if (! GetPFCAlbumInfo(pItem, album) ) return -1;
  ...
  {
    m_pObserver->OnStateChanged(DOWNLOADING_ALBUM_INFO);
    m_pObserver->OnDirectoryChanged(artist.strArtist + " - "+ album.strAlbum);
  }

  // Album work
  int idAlbum = m_musicDatabase.AddAlbum(album);
  m_musicDatabase.SetAlbumInfo(idAlbum, album, album.songs);

  if (album.thumbURL.m_url.size() > 0) {
    CStdString strFullThumbPath;
    URIUtils::CreateArchivePath(strFullThumbPath, "pfc", pItem->GetPath(), album.thumbURL.m_url[0].m_url);
    album.thumbURL.m_url[0].m_url = strFullThumbPath;
    //album.thumbURL.m_url[0].m_url.Insert(0, urlPath.GetWithoutFilename().c_str()); // Laureon: Hack: to insert path before album cover file...
    GetAlbumArtwork(idAlbum, album);
  }
  ...
  {
    m_pObserver->OnStateChanged(DOWNLOADING_ARTIST_INFO);
    m_pObserver->OnDirectoryChanged(artist.strArtist);
  }

  // Check for the artist info
  if ( !m_musicDatabase.GetArtistInfo(album.idArtist,artist) ) {
    m_musicDatabase.SetArtistInfo(album.idArtist, artist);
    GetArtistArtwork(album.idArtist, artist.strArtist, &artist);
    //GetPFCArtistArtwork(pItem);
  }

  m_musicDatabase.CommitTransaction();
  return album.songs.size();
...
//          CStdString strFullSongPath;
//          URIUtils::CreateArchivePath(strFullSongPath, "pfc", strPath, itSong->strFileName); // Laureon: Hack to insert file path in PFC songs...
////          itSong->strFileName = strFullSongPath;
//
//          // Laureon: Duration Check...
//          if (itSong->iDuration <= 0)
//            itSong->iDuration = GetSongDuration(strFullSongPath);
//
//          songs.push_back(*itSong);
//        }
//
//        m_musicDatabase.BeginTransaction();
//        if (m_bStop)
...
//  VECSONGS songs;
  ...
  bCanceled = false;
  m_musicDatabase.Open();
  int idAlbum = m_musicDatabase.GetAlbumByName(strAlbum, strArtist);
  if (idAlbum < 0) return false;
//  if (m_musicDatabase.HasAlbumInfo(params.GetAlbumId()) && m_musicDatabase.GetAlbumInfo(params.GetAlbumId(),album,&songs))
//    CLog::Log(LOGDEBUG,"Album: %s already in DB rescanning ayway", strAlbum.c_str());
...
//  CMusicInfoScraper scraper(info);
  ...
  if (XFILE::CFile::Exists(strNfo))
  {
    ...
    if (result == CNfoFile::FULL_NFO)
    {
      ...
      CAlbum album;
      if (nfoReader.GetDetails(album)) {
        album.thumbURL.m_url[0].m_url.Insert(0,strAlbumPath.c_str()); // Laureon: Hack: to insert path before album cover file...
...
void CMusicInfoScanner::GetAlbumArtwork(long id, const CAlbum &album)
{
  ...
  {
    CStdString thumb;
    m_musicDatabase.GetAlbumThumb(id, thumb);

    if (thumb.IsEmpty() || !XFILE::CFile::Exists(thumb))
    {
      thumb = CThumbnailCache::GetAlbumThumb(album);
      if (CScraperUrl::DownloadThumbnail(thumb,album.thumbURL.m_url[0]))
...
//  CMusicInfoScraper scraper(info);
  ...
  {
    CLog::Log(LOGDEBUG,"Found matching nfo file: %s", strNfo.c_str());
    result = nfoReader.Create(strNfo, ADDON::ADDON_SCRAPER_ARTISTS);
    if (result == CNfoFile::FULL_NFO)
    {
      CLog::Log(LOGDEBUG, "%s Got details from nfo", __FUNCTION__);
      CArtist artist;
...
//    CScraperUrl::DownloadThumbnail(thumb, artist->thumbURL.m_url[0]);
  ...
  if (!CFile::Exists(cachedImage))
  { // check for local fanart
    CLog::Log(LOGDEBUG, "%s looking for fanart for artist %s in folder %s", __FUNCTION__, artistName.c_str(), item2.GetPath().c_str());
