...
void CUtil::GetHomePath(CStdString& strPath, const CStdString& strTarget)
{
  ...
  /* Change strPath accordingly when target is RAVEN_HOME and when INSTALL_PATH
   * and BIN_INSTALL_PATH differ
   */
  CStdString installPath = INSTALL_PATH;
  CStdString binInstallPath = BIN_INSTALL_PATH;
  if (!strTarget.compare("RAVEN_HOME") && installPath.compare(binInstallPath))
  {
...
CStdString CUtil::ValidatePath(const CStdString &path, bool bFixDoubleSlashes /* = false */)
{
  ...
  if (URIUtils::IsURL(path) && 
     (path.Find('%') >= 0 ||
      ...
      path.Left(4).Equals("pfc:") || // Laureon: Added: Filesystem
...
int CUtil::GetMatchingSource(const CStdString& strPath1, VECSOURCES& VECSOURCES, bool& bIsSourceName)
{
  ...
  if (iIndex == -1)
  {
    ...
    if( strPath.Left(6).Equals("rar://") || strPath.Left(6).Equals("zip://") || strPath.Left(6).Equals("pfc://") ) // Laureon: Added: Filesystem
    {
...
void CUtil::ScanForExternalSubtitles(const CStdString& strMovie, std::vector<CStdString>& vecSubtitles )
{
  ...
  for (unsigned int step = 0; step < strLookInPaths.size(); step++)
  {
    if (strLookInPaths[step].length() != 0)
    {
      ...
      CDirectory::GetDirectory(strLookInPaths[step], items,".utf|.utf8|.utf-8|.sub|.srt|.smi|.rt|.txt|.ssa|.text|.ssa|.aqt|.jss|.ass|.idx|.ifo|.rar|.zip|.pfc",false); // Laureon: Added: Filesystem
      ...
      for (int j = 0; j < items.Size(); j++)
      {
        ...
        if (URIUtils::IsRAR(strItem) || URIUtils::IsZIP(strItem) || URIUtils::IsPFC(strItem)) // Laureon: Added: Filesystem
        {
