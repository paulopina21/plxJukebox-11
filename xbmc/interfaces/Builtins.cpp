...
#ifdef IS_PROFESSIONAL   // Laureon: Added: Jukebox Credits System
  { "InsertCredit",               false,   "Jukebox: Inserts a credit" },
  { "RemoveCredit",               false,   "Jukebox: Removes a credit" },
  { "RemoveLastSong",               false,   "Jukebox: Remove the last song and returns 1 coin" },
#endif
...
int CBuiltins::Execute(const CStdString& execString)
{
  ...
  {
#ifdef HAS_CREDITS // Laureon: Added
    CUtil::RunCredits();
#endif
  }

#ifdef IS_PROFESSIONAL // Laureon: Added: Jukebox Credits System
  else if (execute.Equals("insertcredit")) {
    int64_t toinsert = 1;

    //if (params.size())
    //	toinsert = (int64_t) params;
    g_jukeboxManager.GetCoinsManager().InsertCoin(toinsert);
    CLog::Log(LOGDEBUG, "Jukebox: 1 coin inserted");
  }	
  else if (execute.Equals("removecredit")) {
    g_jukeboxManager.GetCoinsManager().RemoveCoin();
    CLog::Log(LOGDEBUG, "Jukebox: 1 coin removed");
  }	
  else if (execute.Equals("removelastsong")) {
    g_jukeboxManager.GetCoinsManager().RemoveLastSong();
    CLog::Log(LOGDEBUG, "Jukebox: 1 music removed and 1 coin inserted");
  }
#endif
  ...
  else if (execute.Equals("exportreport"))
  {
    int iHeading = 0;

    if (params[0].Equals("ablf"))
      iHeading = 70002;
    else if (params[0].Equals("aprova"))
      iHeading = 70003;
    ...
    if (params.size() > 1)
      backupFile = params[1].Equals("true");
    else
      backupFile = CGUIDialogYesNo::ShowAndGetInput(iHeading,70004,70005,-1,70006,70007,cancelled);
    ...
    if (backupFile || !path.IsEmpty() || CGUIDialogFileBrowser::ShowAndGetDirectory(shares, g_localizeStrings.Get(661), path, true))
    {
      if (params[0].Equals("ablf"))
      {
        g_jukeboxManager.GetReportsManager().ExportReport(REPORT_ABLF, path, backupFile);
      }
      else if (params[0].Equals("aprova"))
      {
        g_jukeboxManager.GetReportsManager().ExportReport(REPORT_APROVA, path, backupFile);
      }
    }
  }
