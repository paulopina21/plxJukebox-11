...
#ifdef _LINUX
#define DEFAULT_VISUALISATION "visualization.projectm"
#elif defined(_WIN32)
#ifdef HAS_DX
...
void CGUISettings::Initialize()
{
  ...
  // Pictures settings
#ifndef IS_PROFESSIONAL //Laureon: Added: Removing some config screens 
  AddGroup(0, 1);
...
//  AddGroup(1, 0);
  ...
  AddBool(NULL, "musiclibrary.backgroundupdate", 22001, true);
  AddSeparator(ml,"musiclibrary.sep2");
  AddString(ml, "musiclibrary.musicfiles", 60001, "", BUTTON_CONTROL_STANDARD); // Laureon: Added: Music files in config
...
// Music Player
  CSettingsCategory* mp = AddCategory(3, "musicplayer", 2);
#ifndef IS_PROFESSIONAL
  AddBool(mp, "musicplayer.autoplaynextitem", 489, true);
  AddBool(mp, "musicplayer.queuebydefault", 14084, true);
#else
  AddBool(NULL, "musicplayer.autoplaynextitem", 489, true);
  AddBool(NULL, "musicplayer.queuebydefault", 14084, true);
#endif
  ...
  AddInt(mp, "musicplayer.crossfade", 13314, 7, 0, 1, 15, SPIN_CONTROL_INT_PLUS, MASK_SECS, TEXT_OFF);
  ...
  CSettingsCategory* mf = NULL;//AddCategory(3, "musicfiles", 14081); // Laureon: That was me
...
// My Music settings end

// VIDEOPLAYER START
  CSettingsCategory* vp = AddCategory(3, "videoplayer", 3); // Laureon: Changed: The fisrt param was 3 and the 3rd 14086
...
// VIDEOPLAYER END
  ...
  // SUBTITLES BEGIN
#ifdef _DEBUG
  CSettingsCategory* sub = AddCategory(3, "subtitles", 287);
#else
...
// SUBTITLES END
  ...
  AddString(ao, "audiooutput.audiodevice", 545, "Default", SPIN_CONTROL_TEXT);
#endif
#ifndef IS_JUKEBOX //Laureon: Added: Removing input and power screen config
  ...
  //Laureon: Mod Here! Check lines above...
  AddInt(NULL, "powermanagement.displaysoff", 1450, 0, 0, 1, 1, SPIN_CONTROL_INT_PLUS, MASK_MINS, TEXT_OFF);
  AddInt(NULL, "powermanagement.shutdowntime", 357, 0, 0, 1, 1, SPIN_CONTROL_INT_PLUS, MASK_MINS, TEXT_OFF);
  ...
  // In standalone mode we default to another. // Laureon: Commented shutdown state
...
//  }
...
#ifndef IS_JUKEBOX //Laureon: Added: Removing some config screens
...
#endif
#ifndef IS_JUKEBOX // Laureon: Added removing masterlock config screen
  ...
  AddBool(NULL, "videolibrary.enabled", 418, false);
  ...
  AddString(vdl, "videolibrary.musicfiles", 60001, "", BUTTON_CONTROL_STANDARD); // Laureon: Added: Video files in config
...
//#ifndef IS_PROFESSIONAL //Laureon: Added: Removing some config screens
  ...
  AddString(srv,"services.devicename", 1271, "Raven", EDIT_CONTROL_INPUT);
...
#endif
  AddString(srv,"services.webserverusername",1048, "raven", EDIT_CONTROL_INPUT);
...
//#endif
  ...
  // appearance settings
  AddGroup(7, 480); //Laureon
  CSettingsCategory* laf = NULL;//AddCategory(7,"lookandfeel", 166);
  ...
  AddString(loc, "locale.language",248,"Portuguese (Brazil)", SPIN_CONTROL_TEXT);
  ...
  AddBool(fl, "filelists.showparentdiritems", 13306, false); // Laureon: I was here!!
  ...
  AddDefaultAddon(ss, "screensaver.mode", 356, "screensaver.builtin.slideshow", ADDON_SCREENSAVER);
  AddString(NULL, "screensaver.settings", 21417, "", BUTTON_CONTROL_STANDARD); // Laureon: NULL
  AddString(ss, "screensaver.preview", 1000, "", BUTTON_CONTROL_STANDARD);
  AddSeparator(ss, "screensaver.sep1"); // Laureon: NULL
  AddBool(NULL, "screensaver.usemusicvisinstead", 13392, true); // Laureon: NULL... This will work with my hack in Application.cpp on g_windowManager.Add(new CGUIWindowVisualisation);
  AddBool(NULL, "screensaver.usedimonpause", 22014, false); // Laureon: NULL
...
#ifdef IS_PROFESSIONAL
  AddGroup(8, 72001); //Laureon: New Config: Administrativo
  CSettingsCategory* jboxManager = AddCategory(8,"manager", 72100);
  AddString(jboxManager,"manager.owner", 72101, "PleXus Dynamics", EDIT_CONTROL_INPUT);
  AddString(jboxManager,"manager.contact", 72102, "Paulo Pina", EDIT_CONTROL_INPUT);
  AddString(jboxManager,"manager.contactphone", 72103, "8477-2246", EDIT_CONTROL_INPUT);
  AddSeparator(jboxManager,"manager.sep4");
  AddString(jboxManager,"manager.estatenumber", 72104, "000000000", EDIT_CONTROL_NUMBER_INPUT);
  AddString(jboxManager,"manager.mechanicalcounter", 72105, "0", EDIT_CONTROL_NUMBER_INPUT);

  CSettingsCategory* jboxReports = AddCategory(8,"reports", 72200);
  AddString(jboxReports, "reports.ablf", 72201, "", BUTTON_CONTROL_STANDARD);
  AddString(jboxReports, "reports.aprova", 72202, "", BUTTON_CONTROL_STANDARD);

  CSettingsCategory* jboxEr = AddCategory(8,"jukeboxer", 72300);
  AddBool(jboxEr, "jukeboxer.autorandom", 72301, false);
  AddInt(jboxEr, "jukeboxer.autorandomtime",  72302, 15, 1, 1, 240, SPIN_CONTROL_INT);
  AddSeparator(jboxEr,"jukeboxer.sep1");

  CSettingsCategory* jboxOP = AddCategory(8,"operation", 72400);
//  AddBool(jboxOP, "operation.partymode", 72401, false);
//  AddBool(jboxOP, "operation.partymode.passive", 72402, false);

  map<int,int> opMode;
  opMode.insert(make_pair(72402,JUKEBOX_OPMODE_DEFAULT));
  opMode.insert(make_pair(72403,JUKEBOX_OPMODE_PARTY));
  opMode.insert(make_pair(72404,JUKEBOX_OPMODE_PASSIVE));
  AddInt(jboxOP, "operation.mode", 72401, JUKEBOX_OPMODE_DEFAULT, opMode, SPIN_CONTROL_TEXT);

  AddBool(jboxOP, "operation.partymode_limitlist", 72405, true);
  AddInt(jboxOP, "operation.partymode_maxlistsize",  72406, 15, 0, 1, 20, SPIN_CONTROL_INT);
  AddInt(jboxOP, "operation.partymode_queuewait",  72407, 3, 0, 1, 20, SPIN_CONTROL_INT);
  AddSeparator(jboxEr,"operation.sep1");
  AddInt(jboxOP, "operation.coinsperpulse",  72410, 2, 0, 1, 20, SPIN_CONTROL_INT);
...
bool CGUISettings::SetLanguage(const CStdString &strLanguage)
{
  ...
  if (strNewLanguage != strPreviousLanguage)
  {
    ...
    strLangInfoPath.Format("special://root/language/%s/langinfo.xml", strNewLanguage.c_str());
    ...
    strLanguagePath.Format("special://root/language/%s/strings.xml", strNewLanguage.c_str());
