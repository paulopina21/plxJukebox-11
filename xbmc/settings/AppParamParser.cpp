...
{
  printf("Raven Media Center %s\n", g_infoManager.GetVersion().c_str());
  printf("Copyright (C) 2012 PleXus Dynamics - http://www.plexusdynamics.com.br\n");
  exit(0);
}

void CAppParamParser::DisplayHelp()
{
  printf("Usage: raven [OPTION]... [FILE]...\n\n");
  printf("Arguments:\n");
  printf("  -d <n>\t\tdelay <n> seconds before starting\n");
  printf("  -fs\t\t\tRuns Raven in full screen\n");
  printf("  --standalone\t\tRaven runs in a stand alone environment without a window \n");
  printf("\t\t\tmanager and supporting applications. For example, that\n");
  printf("\t\t\tenables network settings.\n");
  printf("  -p or --portable\tXBMC will look for configurations in install folder instead of ~/.raven\n");
  ...
  printf("  \t\t\t\tspecified file must exist in special://root/system/\n");
  exit(0);
}
