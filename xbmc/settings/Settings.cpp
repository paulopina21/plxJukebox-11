...
void CSettings::Initialize()
{
  ...
  m_bMyVideoNavFlatten = true;
  m_bStartVideoWindowed = false;
  m_bAddonAutoUpdate = true;
  m_bAddonNotifications = false;
  ...
  m_iMyMusicStartWindow = WINDOW_MUSIC_NAV;
  m_iVideoStartWindow = WINDOW_VIDEO_NAV;
...
bool CSettings::LoadSettings(const CStdString& strSettingsFile)
{
  ...
  LoadPlayerCoreFactorySettings("special://root/system/playercorefactory.xml", true);
    ...
    if (profilesDoc.LoadFile(profilesFile))
    {
      ...
      if (rootElement && strcmpi(rootElement->Value(),"profiles") == 0)
      {
        ...
        if (!CDirectory::Exists(defaultDir))
          defaultDir = "special://root/userdata";
        while (pProfile)
        {
...
void CSettings::Initialize()
{
  ...
  m_viewStateMusicNavAlbums.m_viewMode    = 65594;
  m_viewStateMusicNavAlbums.m_sortMethod  = SORT_METHOD_ARTIST;
  m_viewStateMusicNavAlbums.m_sortOrder   = SORT_ORDER_ASC;         
}
...
bool CSettings::LoadSettings(const CStdString& strSettingsFile)
{
  ...
  // mymusic settings
  TiXmlElement *pElement = pRootElement->FirstChildElement("music");
  if (pElement)
  {
    XMLUtils::GetBoolean(pElement, "isscanning", m_bMyMusicIsScanning);
//    TiXmlElement *pChild = pElement->FirstChildElement("playlist");
//    if (pChild)
//    {
//      XMLUtils::GetBoolean(pChild, "repeat", m_bMyMusicPlaylistRepeat);
...
bool CSettings::SaveSettings(const CStdString& strSettingsFile, CGUISettings *localSettings /* = NULL */) const
{
  ...
  TiXmlNode *pNode = pRoot->InsertEndChild(musicNode);
  if (!pNode) return false;
  XMLUtils::SetBoolean(pNode, "isscanning", m_bMyMusicIsScanning);
//  {
//    TiXmlElement childNode("playlist");
//    TiXmlNode *pChild = pNode->InsertEndChild(childNode);
//    if (!pChild) return false;
