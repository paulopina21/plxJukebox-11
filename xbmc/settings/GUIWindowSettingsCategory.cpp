...
//#include "jukebox/JukeboxManager.h" // Laureon: SmartRandom
    ...
    : CGUIWindow(WINDOW_SETTINGS_MYPICTURES, "SettingsCategory.xml")
{
  ...
  m_idRange = 10; // Laureon: Changed to 10 was 8. Changed to accomodate jukebox settings
  m_iScreen = 0;
  m_strOldTrackFormat = "";
...
void CGUIWindowSettingsCategory::UpdateSettings()
{
  for (unsigned int i = 0; i < m_vecSettings.size(); i++)
  {
    ...
    else if (strSetting.Equals("manager.owner")   || strSetting.Equals("manager.contact") ||
             strSetting.Equals("manager.contactphone") )
    {
      CGUIControl *pControl = (CGUIControl *)GetControl(pSettingControl->GetID());
      if (pControl) pControl->SetEnabled(true);
    }
    else if (strSetting.Equals("jukeboxer.autorandomtime"))
    {
      CGUIControl *pControl = (CGUIControl *)GetControl(pSettingControl->GetID());
      if (pControl) pControl->SetEnabled(g_guiSettings.GetBool("jukeboxer.autorandom"));
    }
    else if (strSetting.Equals("operation.mode"))
    {
      g_guiSettings.SetBool("operation.partymode_limitlist",g_guiSettings.GetInt("operation.mode") == JUKEBOX_OPMODE_PARTY);
    }
    else if (strSetting.Equals("operation.partymode_limitlist"))
    {
      CGUIControl *pControl = (CGUIControl *)GetControl(pSettingControl->GetID());
      if (pControl) {
        pControl->SetEnabled(g_guiSettings.GetInt("operation.mode") == JUKEBOX_OPMODE_PARTY);
      }
    }
    else if (strSetting.Equals("operation.partymode_maxlistsize") || strSetting.Equals("operation.partymode_queuewait"))
    {
      CGUIControl *pControl = (CGUIControl *)GetControl(pSettingControl->GetID());
      if (pControl) pControl->SetEnabled(g_guiSettings.GetBool("operation.partymode_limitlist"));
    }
    else if (strSetting.Equals("operation.coinsperpulse"))
    {
      CGUIControl *pControl = (CGUIControl *)GetControl(pSettingControl->GetID());
      if (pControl) pControl->SetEnabled(g_guiSettings.GetInt("operation.mode") == JUKEBOX_OPMODE_DEFAULT);
    }
#endif
#ifdef HAS_LINUX_NETWORK
...
void CGUIWindowSettingsCategory::OnSettingChanged(CBaseSettingControl *pSettingControl)
{
...
#ifdef IS_PROFESSIONAL
  else if ( strSetting.Equals("reports.ablf") )
  {
    CBuiltins::Execute("exportreport(ablf)");
  }
  else if ( strSetting.Equals("reports.aprova") )
  {
    CBuiltins::Execute("exportreport(aprova)");
  }
#endif
  else if (strSetting.Equals("jukeboxer.autorandom")) // Laureon: Jukebox SmartRandom Enable Code
  {
    if (g_guiSettings.GetBool("jukeboxer.autorandom"))
      g_jukeboxManager.GetRandomManager().Start();
    else
      g_jukeboxManager.GetRandomManager().Stop();
  }
  else if (strSetting.Equals("jukeboxer.autorandomtime")) // Laureon: Jukebox SmartRandom Time Code
  {
    g_jukeboxManager.GetRandomManager().SetActionTime(g_guiSettings.GetInt("jukeboxer.autorandomtime"));
  }
  ...
  else if (strSetting.Equals("musiclibrary.musicfiles")) { // Laureon: Added: Music files in config
   g_windowManager.ActivateWindow(WINDOW_MUSIC_FILES); 
  }
  else if (strSetting.Equals("videolibrary.musicfiles")) {
   g_windowManager.ActivateWindow(WINDOW_VIDEO_FILES); 
  }
...
void CGUIWindowSettingsCategory::FillInSubtitleFonts(CSetting *pSetting)
{
  ...
  // find TTF fonts
  {
    ...
    if (CDirectory::GetDirectory("special://root/media/Fonts/", items))
    {
      items.Append(items2);
...
void CGUIWindowSettingsCategory::FillInSoundSkins(CSetting *pSetting)
{
  ...
  //find skins...
  CFileItemList items;
  CDirectory::GetDirectory("special://root/sounds/", items);
...
void CGUIWindowSettingsCategory::FillInLanguages(CSetting *pSetting)
{
  ...
  //find languages...
  CFileItemList items;
  CDirectory::GetDirectory("special://root/language/", items);
